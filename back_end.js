...
function requireModuleAccess(allowedTypes = []) {
  ...
  return (req, res, next) => {
    ...
    if (!userType) {
      return res.status(401).json({ error: 'Tipo de usuario nao informado no header x-user-type.' });
    }
    if (userType === 'ADMIN') return next();
    if (!allowed.includes(userType)) {
      return res.status(403).json({ error: `Acesso negado para o modulo. Perfil atual: ${userType}.` });
    }
    return next();
  };
}
...
// validate/refresh user data (always get latest from DB)
app.post('/user/validate', async (req, res) => {
  const { email } = req.body;
  if (!email) return res.sendStatus(400);
  try {
    const conn = await getConnection();
    const sql = `SELECT * FROM QUALIDADE.GESTAO_CARGAS_TRANSPORTE_USER WHERE email = ?`;
...
app.post('/carregamento/enviar', requireModuleAccess(['CARREGAMENTO', 'SUPERVISOR']), async (req, res) => {
  ...
  try {
    ...
    for (const registro of registros) {
      ...
      // Extrai o sufixo do ID (ex: AGE-2026-112-65743 -> 2026-112-65743)
      const idParts = idDisponibilidade.split('-');
      const idSuffix = idParts.length > 1 ? idParts.slice(1).join('-') : idDisponibilidade;
      const novoIdDisponibilidade = `CGA-${idSuffix}`;
    ...
    if (validationIssues.length > 0) {
      return res.status(400).json({
        error: 'Falha de validacao ao enviar carregamento.',
    ...
    for (const registro of registros) {
      ...
      await conn.query(
        ...
        [
          ...
          String(registro.tipoFrete ?? '').trim(),
...
app.get('/controladoria/processos', requireModuleAccess(['CONTROLADORIA', 'GERENTE', 'SUPERVISOR', 'ADMIN']), async (req, res) => {
  let conn;
  try {
    conn = await getConnection();
    ...
    const rows = await conn.query(
      `SELECT *
       FROM QUALIDADE.GESTAO_CARGAS_TRANSPORTE
       WHERE UPPER(TRIM(STATUS_DISP)) IN ('DIS', 'AGE', 'CGA', 'DVG', 'RCD', 'FNL', 'FNL/DVG')
         AND ID_DISPONIBILIDADE IS NOT NULL
       ORDER BY DATA_REGISTRO DESC`
    );
...
app.patch('/controladoria/processos/:id', requireModuleAccess(['CONTROLADORIA', 'GERENTE', 'ADMIN']), async (req, res) => {
  const idDisponibilidade = String(req.params.id || '').trim();
  const action = String(req.body?.action || 'editar').toLowerCase();
  const userType = String(req.headers['x-user-type'] || '').trim().toUpperCase();
  
  if (!idDisponibilidade) {
    return res.status(400).json({ error: 'ID de disponibilidade invalido.' });
  }
  ...
  try {
    ...
    const existe = await conn.query(
      `SELECT ID_DISPONIBILIDADE, STATUS_DISP
       FROM QUALIDADE.GESTAO_CARGAS_TRANSPORTE
       WHERE ID_DISPONIBILIDADE = ?
       FETCH FIRST 1 ROWS ONLY`,
      [idDisponibilidade]
    ...
    if (!Array.isArray(existe) || existe.length === 0) {
      await conn.close();
      return res.status(404).json({ error: 'Processo nao encontrado na controladoria.' });
    }

    const statusAtual = String(existe[0]?.STATUS_DISP || existe[0]?.statusDisp || '').trim().toUpperCase();
    
    // GERENTE so pode editar apontamentos com status RCD (Carga Recusada)
    if (userType === 'GERENTE' && statusAtual !== 'RCD') {
      await conn.close();
      return res.status(403).json({ error: 'Acesso negado. GERENTE pode editar apenas apontamentos com status RCD (carga recusada).' });
    }
    
    // Apenas CONTROLADORIA, GERENTE e ADMIN podem acessar este endpoint
    if (!['CONTROLADORIA', 'GERENTE', 'ADMIN'].includes(userType)) {
      await conn.close();
      return res.status(403).json({ error: 'Acesso negado. Apenas CONTROLADORIA, GERENTE e ADMIN podem editar processos.' });
    }
    ...
    const colIdDisponibilidade = map.get('IDDISPONIBILIDADE') || 'ID_DISPONIBILIDADE';
    ...
    const colPesoEstimado = map.get('PESOESTIMADO') || 'PESO_ESTIMADO';
    const colPesoLiquido = map.get('PESOLIQUIDO') || 'PESO_LIQUIDO';
    const colPesoBruto = map.get('PESOBRUTO') || 'PESO_BRUTO';
    const colTara = map.get('TARA');
    const colTipoPesagem = map.get('TIPOPESAGEM') || 'TIPO_PESAGEM';
    const colRefugo = map.get('REFUGO');
    const colMotivo = map.get('MOTIVODIVERGENCIA');
    const colObs = map.get('OBSERVACAO') || 'OBSERVACOES';
    ...
    // Tratar fotos (fotosBase64 ou fotos array)
    ...
    if (fotosFromPayload !== undefined && fotosFromPayload !== null) {
      const colFotos = map.get('FOTOS') || 'FOTOS';
      if (colFotos && !usedColumns.has(colFotos)) {
        const fotosBase64Lista = Array.isArray(fotosFromPayload)
          ? fotosFromPayload.filter((item) => hasValue(item)).map((item) => String(item))
          : [];
        const fotosValor = fotosBase64Lista.length > 0 ? JSON.stringify(fotosBase64Lista) : null;
        
        if (fotosValor === null || (typeof fotosValor === 'string' && fotosValor.length > 0)) {
          updates.push(`${colFotos} = ?`);
          params.push(fotosValor || '');
          usedColumns.add(colFotos);
        }
      }
    }
    ...
    if (action === 'recusar') {
      if (!hasValue(req.body?.observacao)) {
        await conn.close();
        return res.status(400).json({ error: 'Motivo da recusa e obrigatorio.' });
      }
      updates.push(`${colStatus} = ?`);
      params.push('RCD');
      addUpdateIfPresent(colObs, 'observacao');
      const message = 'Carga recusada com sucesso! O apontamento foi marcado como RCD.';
    ...
    if (action === 'finalizar') {
      ...
      if (pendentes.length > 0) {
        await conn.close();
        return res.status(400).json({
          error: `Nao e possivel finalizar sem preencher todos os campos obrigatorios. Pendentes: ${pendentes.join(', ')}.`
        });
      }
    }
    ...
    // Para finalizar, AUTOMATICAMENTE determina o status baseado em divergência
    if (action === 'finalizar') {
      // Lê a divergência do registro atual
      const temDivergenciaAtual = readCurrent('TEMOBRIGARGENCIA', 'TEM_DIVERGENCIA') || 
                                   readCurrent('TEMDIVERGENCIA') || '';
      const motivoDivAtual = readCurrent('MOTIVODIVERGENCIA', 'MOTIVO_DIVERGENCIA') || '';
      
      // Se tem divergência marcada (SIM) OU tem motivo de divergência preenchido (e não é "SEM COMENTARIOS")
      const temDivergencia = String(temDivergenciaAtual).trim().toUpperCase() === 'SIM' ||
                            (String(motivoDivAtual).trim() !== '' && String(motivoDivAtual).trim().toUpperCase() !== 'SEM COMENTARIOS');
      
      statusAlvo = temDivergencia ? 'FNL/DVG' : 'FNL';
    }
    // Se nenhum status foi definido pela ação, tenta usar o status do body (fallback)
    ...
    if (statusAlvo) {
      ...
      // Extrai a parte após o prefixo (ex: AGE-2026-115-64161 -> 2026-115-64161)
      const idParts = idDisponibilidade.split('-');
      const idSuffix = idParts.length > 1 ? idParts.slice(1).join('-') : idDisponibilidade;
      // Cria o novo ID com o novo prefixo
      const novoIdDisponibilidade = `${statusAlvo}-${idSuffix}`;
      
      updates.push(`${colIdDisponibilidade} = ?`);
      params.push(novoIdDisponibilidade);
    }
    ...
    const sqlQuery = `UPDATE QUALIDADE.GESTAO_CARGAS_TRANSPORTE
       SET ${updates.join(', ')}
       WHERE ID_DISPONIBILIDADE = ?`;
    ...
    return res.json({
      ...
      message: action === 'finalizar'
        ...
        : action === 'divergencia'
          ? 'Processo marcado com divergencia.'
...
app.post('/apontamento/enviar', requireModuleAccess(['SUPERVISOR']), async (req, res) => {
  ...
  if (!/^[A-Z0-9_]+\.[A-Z0-9_]+$/.test(tableSpec)) {
    return res.status(400).json({ error: 'tableSpec invalido. Use apenas letras/numeros/underscore no formato SCHEMA.TABELA.' });
  }
  const [schema, table] = tableSpec.includes('.') ? tableSpec.split('.') : [null, tableSpec];

  if (!schema || !table) {
    return res.status(400).json({ error: 'tableSpec invalido. Use SCHEMA.TABELA.' });
  }
  ...
  try {
    ...
    const frontendToDb = {
      ...
      operacao: map.get('OPERACAO') || 'OPERACAO',
      prevColheita: map.get('PREVECOLHEITA') || map.get('PREVE_COLHEITA') || 'PREVE_COLHEITA',
      observacao: map.get('OBSERVACAO') || map.get('OBSERVACOES') || 'OBSERVACAO',
      motivoDivergencia: map.get('MOTIVODIVERGENCIA') || map.get('MOTIVO_DIVERGENCIA') || 'MOTIVO_DIVERGENCIA'
    };
    ...
    if (dbColumns.length > 0 && missingColumns.length > 0) {
      await conn.close();
      return res.status(400).json({
        error: 'A tabela nao possui todas as colunas esperadas para insercao.',
        missingColumns,
        table: `${schema}.${table}`
      });
    }
    ...
    if (dbColumnsMeta.length > 0) {
      ...
      if (blockedRequiredColumns.length > 0) {
        await conn.close();
        return res.status(400).json({
          error: 'A tabela possui colunas obrigatorias sem valor no payload.',
          table: `${schema}.${table}`,
    ...
    for (const [index, registro] of registros.entries()) {
      const columns = [
        ...
        frontendToDb.status,
        frontendToDb.operacao,
        frontendToDb.prevColheita,
        frontendToDb.observacao,
        frontendToDb.motivoDivergencia
      ];
      ...
      const valueByColumn = {
        ...
        [frontendToDb.operacao]: registro.operacao,
        [frontendToDb.prevColheita]: registro.prevColheita,
        [frontendToDb.observacao]: 'SEM COMENTARIOS',
        [frontendToDb.motivoDivergencia]: 'SEM COMENTARIOS'
      };
      ...
      const params = columns.map(col => {
        ...
        } else if (col === frontendToDb.estoqueTotal || col === frontendToDb.prevColheita) {
        ...
        } else if (col === frontendToDb.status || col === frontendToDb.operacao) {
        ...
        if ((dbType.includes('NUMBER') || dbType.includes('FLOAT') || dbType.includes('DECIMAL') || dbType.includes('INTEGER')) && hasValue(rawValue) && converted === null) {
          paramIssues.push({
            column: col,
            value: rawValue,
            issue: 'Valor numerico invalido para coluna numerica'
          });
        }
        ...
        if ((col === frontendToDb.criadoEm || dbType.includes('DATE') || dbType.includes('TIMESTAMP')) && hasValue(rawValue) && converted === null) {
          paramIssues.push({
            column: col,
            value: rawValue,
            issue: 'Data invalida. Use formato dd/mm/aaaa, hh:mm:ss'
          });
        }
      ...
      if (paramIssues.length > 0) {
        await conn.close();
        return res.status(400).json({
          error: `Falha de validacao no registro ${index + 1}`,
          table: `${schema}.${table}`,
          paramIssues,
