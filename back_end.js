...
app.post('/carregamento/enviar', requireModuleAccess(['CARREGAMENTO', 'SUPERVISOR']), async (req, res) => {
  ...
  try {
    ...
    for (const registro of registros) {
      ...
      const tara = hasValue(registro.tara) ? String(registro.tara).trim() : null;
      const pesoBruto = hasValue(registro.pesoBruto) ? String(registro.pesoBruto).trim() : null;
      const pesoLiquido = hasValue(registro.pesoLiquido) ? String(registro.pesoLiquido).trim() : null;
      const motivoDivergencia = hasValue(registro.motivoDivergencia) ? String(registro.motivoDivergencia).trim() : null;
      const observacao = hasValue(registro.observacao) ? String(registro.observacao).trim() : null;
      const preveColheita = hasValue(registro.preveColheita) ? String(registro.preveColheita).trim() : null;
      ...
      const taraMeta = dbMetaByColumn.get('TARA');
      const pesoBrutoMeta = dbMetaByColumn.get('PESO_BRUTO');
      const pesoLiquidoMeta = dbMetaByColumn.get('PESO_LIQUIDO');
      const motivoDivergenciaMeta = dbMetaByColumn.get('MOTIVO_DIVERGENCIA');
      const observacaoMeta = dbMetaByColumn.get('OBSERVACAO') || dbMetaByColumn.get('OBSERVACOES');
      const preveColheitaMeta = dbMetaByColumn.get('PREVE_COLHEITA');
      ...
      const taraType = String(taraMeta?.type || '').toUpperCase();
      const pesoBrutoType = String(pesoBrutoMeta?.type || '').toUpperCase();
      const pesoLiquidoType = String(pesoLiquidoMeta?.type || '').toUpperCase();
      const motivoDivergenciaType = String(motivoDivergenciaMeta?.type || '').toUpperCase();
      const observacaoType = String(observacaoMeta?.type || '').toUpperCase();
      const preveColheitaType = String(preveColheitaMeta?.type || '').toUpperCase();
      ...
      const taraMax = Number(taraMeta?.length || 0);
      const pesoBrutoMax = Number(pesoBrutoMeta?.length || 0);
      const pesoLiquidoMax = Number(pesoLiquidoMeta?.length || 0);
      const motivoDivergenciaMax = Number(motivoDivergenciaMeta?.length || 0);
      const observacaoMax = Number(observacaoMeta?.length || 0);
      const preveColheitaMax = Number(preveColheitaMeta?.length || 0);
      ...
      if (!taraType.includes('CLOB') && !taraType.includes('NCLOB') && !taraType.includes('BLOB') && Number.isFinite(taraMax) && taraMax > 0 && typeof tara === 'string' && tara.length > taraMax) {
        validationIssues.push({
          idDisponibilidade,
          column: 'TARA',
          maxLength: taraMax,
          actualLength: tara.length,
          message: 'TARA excede o tamanho permitido no banco.'
        });
      }

      if (!pesoBrutoType.includes('CLOB') && !pesoBrutoType.includes('NCLOB') && !pesoBrutoType.includes('BLOB') && Number.isFinite(pesoBrutoMax) && pesoBrutoMax > 0 && typeof pesoBruto === 'string' && pesoBruto.length > pesoBrutoMax) {
        validationIssues.push({
          idDisponibilidade,
          column: 'PESO_BRUTO',
          maxLength: pesoBrutoMax,
          actualLength: pesoBruto.length,
          message: 'PESO_BRUTO excede o tamanho permitido no banco.'
        });
      }

      if (!pesoLiquidoType.includes('CLOB') && !pesoLiquidoType.includes('NCLOB') && !pesoLiquidoType.includes('BLOB') && Number.isFinite(pesoLiquidoMax) && pesoLiquidoMax > 0 && typeof pesoLiquido === 'string' && pesoLiquido.length > pesoLiquidoMax) {
        validationIssues.push({
          idDisponibilidade,
          column: 'PESO_LIQUIDO',
          maxLength: pesoLiquidoMax,
          actualLength: pesoLiquido.length,
          message: 'PESO_LIQUIDO excede o tamanho permitido no banco.'
        });
      }

      if (!motivoDivergenciaType.includes('CLOB') && !motivoDivergenciaType.includes('NCLOB') && !motivoDivergenciaType.includes('BLOB') && Number.isFinite(motivoDivergenciaMax) && motivoDivergenciaMax > 0 && typeof motivoDivergencia === 'string' && motivoDivergencia.length > motivoDivergenciaMax) {
        validationIssues.push({
          idDisponibilidade,
          column: 'MOTIVO_DIVERGENCIA',
          maxLength: motivoDivergenciaMax,
          actualLength: motivoDivergencia.length,
          message: 'MOTIVO_DIVERGENCIA excede o tamanho permitido no banco.'
        });
      }

      if (!observacaoType.includes('CLOB') && !observacaoType.includes('NCLOB') && !observacaoType.includes('BLOB') && Number.isFinite(observacaoMax) && observacaoMax > 0 && typeof observacao === 'string' && observacao.length > observacaoMax) {
        validationIssues.push({
          idDisponibilidade,
          column: 'OBSERVACAO',
          maxLength: observacaoMax,
          actualLength: observacao.length,
          message: 'OBSERVACAO excede o tamanho permitido no banco.'
        });
      }

      if (!preveColheitaType.includes('CLOB') && !preveColheitaType.includes('NCLOB') && !preveColheitaType.includes('BLOB') && Number.isFinite(preveColheitaMax) && preveColheitaMax > 0 && typeof preveColheita === 'string' && preveColheita.length > preveColheitaMax) {
        validationIssues.push({
          idDisponibilidade,
          column: 'PREVE_COLHEITA',
          maxLength: preveColheitaMax,
          actualLength: preveColheita.length,
          message: 'PREVE_COLHEITA excede o tamanho permitido no banco.'
        });
      }
...
app.post('/logistica/enviar', requireModuleAccess(['SUPERVISOR', 'LOGISTICA']), async (req, res) => {
  ...
  try {
    ...
    for (const registro of registros) {
      ...
      const observacao = String(registro.observacao ?? '').trim() || 'SEM COMENTARIOS';
...
app.get('/controladoria/processos', requireModuleAccess(['CONTROLADORIA', 'SUPERVISOR', 'GERENTE']), async (req, res) => {
  let conn;
  try {
    conn = await getConnection();
...
app.patch('/controladoria/processos/:id', requireModuleAccess(['CONTROLADORIA', 'GERENTE']), async (req, res) => {
